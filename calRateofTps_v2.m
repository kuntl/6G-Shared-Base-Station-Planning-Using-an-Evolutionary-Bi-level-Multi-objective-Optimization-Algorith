...
%         end
        ...
        for j = 1:1:op_rent_num_i
            ...
            for k = 1:1:n
                %aaaa(k,1) = find(Tp_bs_distance_index(:,:,tps_j(k,1)) == j);
...
%                         end
                    ...
                    exceed = tps_rate_i_down(tps_j)>=rate_down_min;
...
%                         end
                    ...
                    exceed = tps_rate_i_up(tps_j)>=rate_up_min;
                    tmp_gain(exceed) = -tmp_gain(exceed);
                    for t = 1:1:2
                        if all(tmp_gain < 0)
                            break
